...
import com.bank.loans.models.LoanInterestStatistics;
...
public interface InstallmentRepository extends BaseRepository<Installment, Long> {
    ...
    @Query(value = "SELECT new com.bank.loans.models.LoanInterestStatistics(i.currency, sum(i.interestAmount)) FROM Installment i WHERE i.paid = true AND i.paidDate > ?1 AND i.paidDate < ?2 GROUP BY i.currency")
